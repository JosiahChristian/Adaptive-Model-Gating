...
from experiment_017 import calibrate_cumulative_thresholds,generate_experiment_017_stream
...
class TestExperiment019Frozen(unittest.TestCase):
 ...
 def test_targeted_round_uses_only_two_blocks(self):
  s=generate_experiment_017_stream(19998,'drift_ab_gain050',1.0);t=inject_targeted_round5(s,('a','b'))
 ...
 def test_abstention_falls_back_operationally(self):
  ...
  tau=calibrate_tau();k=calibrate_kappa();k3=calibrate_kappa3();la=calibrate_lambda_anchor();lb,lab=calibrate_dual_anchor_thresholds();lc,lac,lbc=calibrate_anchor_c_thresholds();lambdas=calibrate_lambda_probe_rounds();mu,nu=calibrate_cumulative_thresholds();mu5,nu5=calibrate_round5_thresholds()
  forced_nu=(float('inf'),)*4
  a=run_experiment_019_strategy(19998,'drift_ab_gain0125',1.0,'targeted_replicated_selective_cumulative_provenance_quorum',tau,k,k3,la,lb,lc,lab,lac,lbc,lambdas,mu,forced_nu,mu5,nu5,float('inf'),float('inf'))
  b=run_experiment_019_strategy(19998,'drift_ab_gain0125',1.0,'triad_persistence',tau,k,k3,la,lb,lc,lab,lac,lbc,lambdas,mu,forced_nu,mu5,nu5,float('inf'),float('inf'))
  self.assertTrue(all(r['provenance_abstain']==1 for r in a))
