...
from experiment_016 import ROUND_AMPLITUDES,groups_from_edges,partition_matches,probe_energy
from experiment_017 import (
    calibrate_cumulative_thresholds,generate_experiment_017_stream,infer_selective_cumulative,
)
...
def cumulative5_statistics(stream):
    ...
    Rs[5]=round5_response_matrix(stream)
    denom=sqrt(sum(d*d for d in ALL_AMPLITUDES))
    C={}
    for i in 'abc':
        for j in 'abc':
...
def calibrate_round5_thresholds():
    ...
    for seed in ROUND5_CALIBRATION_SEEDS:
        s=generate_experiment_018_stream(seed,'healthy',0.0,gain_override=0.0)
        C,_=cumulative5_statistics(s);vals.extend(C.values())
    return empirical_quantile(vals,0.99),empirical_quantile(vals,0.999)


def infer_round5(stream,mu5,nu5):
    C,Rs=cumulative5_statistics(stream);pairs=(('a','b'),('a','c'),('b','c'))
    Q={(i,j):min(C[(i,j)],C[(j,i)]) for i,j in pairs}
    edges=[(i,j) for i,j in pairs if Q[(i,j)]>mu5]
    ...
    qualified=int(structural and Q[edges[0]]>nu5)
...
def infer_replicated_selective(stream,mu,nu,mu5,nu5):
    ...
    if accepted:
        return inferred,executed,stop,1,0,candidate,0,None
    r5=infer_round5(stream,mu5,nu5)
    executed=dict(executed);executed[5]=r5;candidate=r5[0]
...
def _annotation(stream,mu,nu,mu5,nu5,executed,accepted,stop,abstain,candidate,gate_groups,rescued):
    ...
    out={'probe_gain':stream['probe_gain'],'probe_stop_round':stop,'probe_energy':energy,'probe_block_count':3*len(executed),
         'probe_max_amplitude':ALL_AMPLITUDES[(stop or max(executed))-1] if executed else 0.0,'provenance_accepted':accepted,
         'provenance_abstain':abstain,'accepted_partition_correct':partition_matches(gate_groups) if accepted else '',
         'candidate_partition_correct':partition_matches(candidate) if candidate else '','round5_executed':int(5 in executed),'round5_rescued_acceptance':rescued,
         'round5_rescued_correct':partition_matches(gate_groups) if rescued else ''}
    for r in range(1,5):out[f'mu_probe_{r}']=mu[r-1];out[f'nu_probe_{r}']=nu[r-1]
    out['mu_probe_5']=mu5;out['nu_probe_5']=nu5
    if 5 in executed:
        groups,C,Q,edges,structural,qualified,Rs=executed[5]
        out['cumulative_r5_structural']=structural;out['cumulative_r5_qualified']=qualified;out['cumulative_r5_edges']=';'.join(''.join(x) for x in edges)
...
def _run_group_gate(seed,label,tau,k3,la,lb,lc,lab,lac,lbc,stream,ann,gate_groups):
    ...
    for t in range(INITIAL_FIT_END+1,N_STEPS+1):
        ...
        m={x:int(h[x][t] is not None and h[x][t]>l) for x,l in [('a',la),('b',lb),('c',lc)]};dis={x:int(h[x][t] is not None and h[x][t]>l) for x,l in [('ab',lab),('ac',lac),('bc',lbc)]}
...
def _fallback(seed,family,magnitude,tau,kappa,k3,la,lb,lc,lab,lac,lbc,lambdas,mu,nu,mu5,nu5,stream,executed,candidate):
    rows=run_experiment_017_strategy(seed,family,magnitude,'triad_persistence',tau,kappa,k3,la,lb,lc,lab,lac,lbc,lambdas,mu,nu)
    ann=_annotation(stream,mu,nu,mu5,nu5,executed,0,0,1,candidate,None,0)
    for row in rows:
        row['strategy']='replicated_selective_cumulative_provenance_quorum';row.update(ann)
    return rows


def run_experiment_018_strategy(seed,family,magnitude,strategy,tau,kappa,k3,la,lb,lc,lab,lac,lbc,lambdas,mu,nu,mu5,nu5):
    if strategy!='replicated_selective_cumulative_provenance_quorum':
        return run_experiment_017_strategy(seed,family,magnitude,strategy,tau,kappa,k3,la,lb,lc,lab,lac,lbc,lambdas,mu,nu)
    stream=generate_experiment_018_stream(seed,family,magnitude);label=f'experiment018_{family}_{magnitude:.3f}'
    inferred,executed,stop,accepted,abstain,candidate,rescued,r5=infer_replicated_selective(stream,mu,nu,mu5,nu5)
    ...
    ann=_annotation(stream,mu,nu,mu5,nu5,executed,accepted,stop,abstain,candidate,inferred,rescued)
    return _run_group_gate(seed,label,tau,k3,la,lb,lc,lab,lac,lbc,stream,ann,inferred)
