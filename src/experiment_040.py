...
LOCAL_MIXTURE_STRATEGY='local_gaussian_student_t3_contamination_context_composed_risk_gate'
STRATEGIES=(LOCAL_MIXTURE_STRATEGY,COMPOSED_STRATEGY,ROBUST_STRATEGY,MODEL_AVERAGED_STRATEGY,HUBER_STRATEGY,RADIAL_HUBER_STRATEGY,TRIAD)
...
def generate_experiment_040_stream(seed,c):return generate_experiment_036_stream(seed,c)

@contextmanager
def bind_experiment_040_stream(stream):
...
def _log_gaussian_block(q,det):
 return -log(2.0*pi)-0.5*log(det)-0.5*q

def _log_student_t3_block(q_sigma,det_sigma):
 # covariance-matched t_3 uses scatter Sigma/3.  For d=2, q_scatter/nu=q_sigma.
 nu=STUDENT_DF;d=2.0
 logdet_scatter=log(det_sigma)-2.0*log(3.0)
 return (lgamma((nu+d)/2.0)-lgamma(nu/2.0)-0.5*d*log(nu*pi)-0.5*logdet_scatter-0.5*(nu+d)*log(1.0+q_sigma))

def _log_local_mixture(v,var,cov):
 det,qs=_block_mahalanobis(v,var,cov);total=0.0
 for q in qs:
  lg=_log_gaussian_block(q,det);lt=_log_student_t3_block(q,det)
  total+=_lse2(log(GAUSSIAN_WEIGHT)+lg,log(STUDENT_WEIGHT)+lt)
 return total

def posterior_local_mixture(y,var,cov):
 y=tuple(float(x) for x in y);logs={'H_null':log(0.25)+_log_local_mixture(y,var,cov)}
 for h,u in TOPOLOGY_DIRECTIONS.items():
  ...
  for idx,beta in enumerate(BETA_GRID):
   ...
   terms.append(_log_local_mixture(r,var,cov)+_log_halfnormal(beta)+log(w*BETA_STEP))
...
def infer_local_mixture(stream):
 _,sigma_hat=diagnostic_noise_factor(stream);path=[]
 for stage in range(1,6):
  y=directed_stage(stream,stage);var,cov=covariance_terms(sigma_hat,stage);post=posterior_local_mixture(y,var,cov);cand=max(UNIQUE,key=lambda h:post[h]);p=float(post[cand])
...
def run_experiment_040_strategy(seed,c,strategy,vals):
 ...
 stream=generate_experiment_040_stream(seed,c)
 if strategy!=LOCAL_MIXTURE_STRATEGY:
  with bind_experiment_040_stream(stream):rows=run_experiment_032_strategy(seed,c,strategy,vals)
 else:
  s=inject_symmetric_round5(stream);groups,accepted,abstain,stop,path=infer_local_mixture(s);ann=_annotation(s,groups,accepted,abstain,stop,path);tau,kappa,k3,la,lb,lc,lab,lac,lbc,*_=vals
  if abstain:
   from experiment_010 import run_triad_persistence_on_stream
   rows=run_triad_persistence_on_stream(seed,f'experiment040_{c["label"]}',tau,k3,stream)
   for r in rows:r['strategy']=LOCAL_MIXTURE_STRATEGY;r.update(ann)
  else:
   rows=_run_composed_gate(seed,f'experiment040_{c["label"]}',tau,k3,la,lb,lc,lab,lac,lbc,s,ann,groups)
   for r in rows:r['strategy']=LOCAL_MIXTURE_STRATEGY
 for r in rows:
  r['experiment040_cell']=c['label'];r['experiment040_noise_family']=c['noise_family'];r['local_mixture_gaussian_weight']=GAUSSIAN_WEIGHT;r['local_mixture_student_weight']=STUDENT_WEIGHT;r['local_mixture_student_df']=STUDENT_DF;r['robust_beta_step']=BETA_STEP;r['robust_beta_max']=BETA_MAX
 return rows

def evaluate_local_mixture_posterior(seed,c):
 s=inject_symmetric_round5(generate_experiment_040_stream(seed,c));_,sigma_hat=diagnostic_noise_factor(s);out=[]
 for stage in range(1,6):
  y=directed_stage(s,stage);var,cov=covariance_terms(sigma_hat,stage);post=posterior_local_mixture(y,var,cov);top=max(HYPOTHESES,key=lambda h:post[h])
