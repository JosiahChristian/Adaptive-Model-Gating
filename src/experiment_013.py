...
def run_quorum(seed,label,tau,k3,la,lb,lc,lab,lac,lbc,stream,family,provenance):
 ...
 for t in range(INITIAL_FIT_END+1,N_STEPS+1):
  ...
  cross=d['provenance_cross_consistent'] if provenance else d['raw_cross_consistent'];votes=d['provenance_mismatch_votes'] if provenance else d['raw_mismatch_votes'];suspect=int(d['triad_consistent'] and cross and votes>=2);streak=(streak+1 if rmse is not None and rmse>tau else 0);ready=streak>=PERSISTENCE_COUNT;veto=int(ready and (pbad or suspect));adapt=int(ready and not veto)
  if ready: streak=0
