...
OPERATIVE_SPEC_ISSUE=110
DIRECTIONAL_GAUSSIAN_STRATEGY='directional_persistence_gaussian_posterior_context_composed_risk_gate'
STRATEGIES=(DIRECTIONAL_GAUSSIAN_STRATEGY,COMPOSED_STRATEGY,REPLICATED_GAUSSIAN_STRATEGY,ROBUST_STRATEGY,MODEL_AVERAGED_STRATEGY,RADIAL_HUBER_STRATEGY,LOCAL_MIXTURE_STRATEGY,LOCAL_CAUCHY_STRATEGY,LOCAL_GAUSSIAN_GROSS_STRATEGY,TRIAD)
...
def generate_experiment_044_stream(seed,c):return generate_experiment_036_stream(seed,c)

@contextmanager
def bind_experiment_044_stream(stream):
...
def infer_directional_gaussian(stream):
 _,sigma_hat=diagnostic_noise_factor(stream);path=[];prev_candidate=None
 for stage in range(1,6):
  ...
  cand=max(UNIQUE,key=lambda h:post[h]);p=float(post[cand]);cross=int(p>=ACCEPT_THRESHOLD);same_prev=int(stage>=2 and prev_candidate==cand);confirmed=int(stage>=2 and cross and same_prev)
  row={'stage':stage,'candidate':cand,'candidate_posterior':p,'posterior_error_risk':1.0-p,'expected_wrong_action_loss':WRONG_COST*(1.0-p),'posterior':post,'variance':var,'shared_covariance':cov,'mahalanobis_zero':quad,'threshold_crossing':cross,'previous_candidate':prev_candidate or '','same_as_previous_candidate':same_prev,'directional_confirmed':confirmed}
  path.append(row)
  if confirmed:return groups_from_edges([EDGE[cand]]),1,0,stage,path
  prev_candidate=cand
 return None,0,1,0,path

def _directional_annotation(path,accepted,stop):
 out={'directional_rule':'current candidate posterior >=0.99 and same unique non-null candidate at immediately preceding stage','directional_previous_probability_thresholded':0,'directional_confirmed':int(bool(accepted))}
 for s in path:
  r=s['stage'];out[f'directional_r{r}_candidate']=s['candidate'];out[f'directional_r{r}_candidate_p']=s['candidate_posterior'];out[f'directional_r{r}_threshold_crossing']=s['threshold_crossing'];out[f'directional_r{r}_same_as_previous']=s['same_as_previous_candidate'];out[f'directional_r{r}_confirmed']=s['directional_confirmed']
 if accepted:
  cur=path[stop-1];prev=path[stop-2]
  out['directional_previous_candidate']=prev['candidate'];out['directional_previous_posterior']=prev['candidate_posterior'];out['directional_accept_candidate']=cur['candidate'];out['directional_accept_posterior']=cur['candidate_posterior']
 else:
  out['directional_previous_candidate']='';out['directional_previous_posterior']='';out['directional_accept_candidate']='';out['directional_accept_posterior']=''
 return out

def run_experiment_044_strategy(seed,c,strategy,vals):
 ...
 stream=generate_experiment_044_stream(seed,c)
 if strategy!=DIRECTIONAL_GAUSSIAN_STRATEGY:
  with bind_experiment_044_stream(stream):rows=run_experiment_032_strategy(seed,c,strategy,vals)
 else:
  s=inject_symmetric_round5(stream);groups,accepted,abstain,stop,path=infer_directional_gaussian(s);ann=_annotation(s,groups,accepted,abstain,stop,path);ann.update(_directional_annotation(path,accepted,stop));tau,kappa,k3,la,lb,lc,lab,lac,lbc,*_=vals
  if abstain:
   from experiment_010 import run_triad_persistence_on_stream
   rows=run_triad_persistence_on_stream(seed,f'experiment044_{c["label"]}',tau,k3,stream)
   for r in rows:r['strategy']=DIRECTIONAL_GAUSSIAN_STRATEGY;r.update(ann)
  else:
   rows=_run_composed_gate(seed,f'experiment044_{c["label"]}',tau,k3,la,lb,lc,lab,lac,lbc,s,ann,groups)
   for r in rows:r['strategy']=DIRECTIONAL_GAUSSIAN_STRATEGY
 for r in rows:
  r['experiment044_cell']=c['label'];r['experiment044_noise_family']=c['noise_family'];r['experiment044_directional_rule']='adjacent-same-candidate-current-p>=0.99'
 return rows

def evaluate_gaussian_posterior(seed,c):
 s=inject_symmetric_round5(generate_experiment_044_stream(seed,c));_,sigma_hat=diagnostic_noise_factor(s);out=[]
 for stage in range(1,6):
