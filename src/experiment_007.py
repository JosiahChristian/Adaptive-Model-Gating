from __future__ import annotations
from random import Random
from adaptive_model_gating import BASELINE_A, EVENT_T, N_STEPS, run_strategy_on_stream

TRANSIENT_INPUT_CORRUPTION_DURATION = 20


def generate_input_sensor_corruption_stream(seed, sigma_x, transient_duration=None):
    if sigma_x < 0:
        raise ValueError("sigma_x must be nonnegative")
    if transient_duration is not None and transient_duration <= 0:
        raise ValueError("transient_duration must be positive or None")

    rng = Random(seed)
    x_true = [0.0] * (N_STEPS + 1)
    x_obs = [0.0] * (N_STEPS + 1)
    ys = [0.0] * (N_STEPS + 1)
    a_values = [BASELINE_A] * (N_STEPS + 1)
    sigma_values = [0.0] * (N_STEPS + 1)
    physical_eps = [0.0] * (N_STEPS + 1)
    sensor_unit = [0.0] * (N_STEPS + 1)

    for t in range(1, N_STEPS + 1):
        x_true[t] = 0.8 * x_true[t - 1] + rng.gauss(0, 0.5)
        physical_eps[t] = rng.gauss(0, 0.5)
        sensor_unit[t] = rng.gauss(0, 1.0)
        active = t >= EVENT_T and (
            transient_duration is None or t < EVENT_T + transient_duration
        )
        sigma_values[t] = sigma_x if active else 0.0
        x_obs[t] = x_true[t] + sigma_values[t] * sensor_unit[t]
        ys[t] = BASELINE_A * x_true[t] + physical_eps[t]

    return x_obs, ys, x_true, a_values, sigma_values, physical_eps, sensor_unit


def run_input_sensor_corruption_strategy(seed, sigma_x, transient_duration, strategy, tau):
    x_obs, ys, x_true, a_values, sigma_values, physical_eps, sensor_unit = (
        generate_input_sensor_corruption_stream(seed, sigma_x, transient_duration)
    )
    duration_label = (
        "persistent" if transient_duration is None else f"transient_d_{transient_duration}"
    )
    rows = run_strategy_on_stream(
        seed,
        f"input_sensor_sigma_{sigma_x:.2f}_{duration_label}",
        strategy,
        tau,
        x_obs,
        ys,
        a_values,
    )
    for row in rows:
        t = row["t"]
        row["x_true"] = x_true[t]
        row["x_obs"] = x_obs[t]
        row["physical_epsilon"] = physical_eps[t]
        row["sensor_unit_noise"] = sensor_unit[t]
        row["true_sigma_x"] = sigma_values[t]
        latent_hat = row["slope_before"] * x_true[t] + row["intercept_before"]
        row["y_hat_latent"] = latent_hat
        latent_error = ys[t] - latent_hat
        row["latent_input_error"] = latent_error
        row["latent_input_sq_error"] = latent_error * latent_error
    return rows
