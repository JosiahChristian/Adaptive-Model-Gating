...
from run_experiment_052 import CELLS,STRATEGIES,SEEDS,AUDIT,summary,write_csv,calibration_values,run_experiment_052_strategy
 ...
 with (out/'audit.jsonl').open('w',encoding='utf-8') as af:
  for seed in SEEDS:
   for st in STRATEGIES:
    rows=run_experiment_052_strategy(seed,c,st,vals);summ.append(summary(rows,c))
    if seed in AUDIT:
     for r in rows:af.write(json.dumps(r,separators=(',',':'))+'\n')
 write_csv(out/'summary.csv',summ);(out/'meta.json').write_text(json.dumps({'cell_index':a.cell_index,'label':c['label'],'summary_rows':len(summ),'experiment':52},indent=2))
