...
from experiment_041 import LOCAL_CAUCHY_STRATEGY,GAUSSIAN_WEIGHT,CAUCHY_WEIGHT,CAUCHY_DF,STRATEGIES,CELLS,run_experiment_041_strategy,evaluate_local_cauchy_posterior
from run_experiment_021 import calibrations
SEEDS=range(41000,42000);AUDIT=set(range(41000,41005));Z=1.6448536269514722
...
def report_from(rows,posterior_rows):
 ...
 for c in CELLS:
  ...
  L,G,T,M,CW,RH,P=rr[LOCAL_CAUCHY_STRATEGY],rr[COMPOSED_STRATEGY],rr[ROBUST_STRATEGY],rr[MODEL_AVERAGED_STRATEGY],rr[HUBER_STRATEGY],rr[RADIAL_HUBER_STRATEGY],rr[LOCAL_MIXTURE_STRATEGY]
  ...
  if c['noise_family']=='gaussian' and (L['coverage']<G['coverage']-.03 or losses[LOCAL_CAUCHY_STRATEGY]>losses[COMPOSED_STRATEGY]+.05):H['H5']=False
  ...
  if c['label'] in HIGH_CONTAM and (L['wrong_acceptance']>P['wrong_acceptance']-.003 or L['wrong_wilson_upper_95']>.01 or (L['precision'] or 0)<.99):H['H7']=False
  if (c['noise_family']=='gaussian' or c['label'] in HIGH_CONTAM) and L['coverage']<P['coverage']-.02:H['H8']=False
  if losses[LOCAL_CAUCHY_STRATEGY]>losses[TRIAD]+.20:H['H9']=False
  fb=0
  if sum(int(float(x['causal_violation_count'])) for x in qs[LOCAL_CAUCHY_STRATEGY]) or sum(int(float(x['triad_veto_adapt_violations'])) for x in qs[LOCAL_CAUCHY_STRATEGY]):H['H10']=False
  for s in SEEDS:
   a,t=by[(s,LOCAL_CAUCHY_STRATEGY)],by[(s,TRIAD)]
 ...
 H['H11']=bool(GAUSSIAN_WEIGHT==.95 and CAUCHY_WEIGHT==.05 and CAUCHY_DF==1.0 and BETA_STEP==.01 and BETA_MAX==1.20 and ACCEPT_THRESHOLD==.99 and WRONG_COST==100.0 and FALLBACK_COST==1.0)
 return {'evaluation_seeds':[41000,41999],'n_seeds_per_cell':1000,'cell_count':16,'strategies':STRATEGIES,'audit_seeds':sorted(AUDIT),'bootstrap_seed':41041,'bootstrap_resamples':10000,'hypotheses':H,'accept_threshold':ACCEPT_THRESHOLD,'wrong_action_cost':WRONG_COST,'fallback_cost':FALLBACK_COST,'local_mixture_weights':{'gaussian':GAUSSIAN_WEIGHT,'cauchy':CAUCHY_WEIGHT},'cauchy_df':CAUCHY_DF,'cauchy_scatter':'Sigma inherited from Experiment-028 analytic block covariance','cauchy_density':'[2*pi*|Sigma|^(1/2)]^-1 * (1 + r^T Sigma^-1 r)^(-3/2)','likelihood':'proper independent per-block 0.95 Gaussian + 0.05 multivariate-Cauchy gross-error mixture on three inherited 2-D directed-response covariance blocks','latent_marginalization':'component marginalized independently within each 2-D block; no posterior component selection','context_rule':'Experiment-031 current-time causal context vote composed by Experiment-032','amplitude_prior':'HalfNormal inherited BETA_SCALE; trapezoid beta grid 0..1.20 step 0.01','vulnerable_experiment035_cells':sorted(VULNERABLE_035),'gross_contamination_repair_cells':sorted(HIGH_CONTAM),'no_tuning':True,'code_commit':os.environ.get('GITHUB_SHA','unknown'),'cells':out}
