...
from experiment_039 import RADIAL_HUBER_STRATEGY,RADIAL_C,STRATEGIES,CELLS,run_experiment_039_strategy,evaluate_radial_huber_posterior
from run_experiment_021 import calibrations
SEEDS=range(39000,40000);AUDIT=set(range(39000,39005));Z=1.6448536269514722
...
def report_from(rows,posterior_rows):
 ...
 for c in CELLS:
  ...
  R,G,T,M,CW=rr[RADIAL_HUBER_STRATEGY],rr[COMPOSED_STRATEGY],rr[ROBUST_STRATEGY],rr[MODEL_AVERAGED_STRATEGY],rr[HUBER_STRATEGY]
  if R['wrong_wilson_upper_95']>.01:H['H2']=False
  if (R['precision'] or 0)<.99:H['H3']=False
  if float(c['noise_scale'])==1.0:
   req=.90 if float(c['gain'])==.50 else .85
   if R['coverage']<req:H['H4']=False
  losses={st:avg(qs[st],'operational_loss_401_600') for st in STRATEGIES}
  if c['noise_family']=='gaussian' and (R['coverage']<G['coverage']-.03 or losses[RADIAL_HUBER_STRATEGY]>losses[COMPOSED_STRATEGY]+.05):H['H5']=False
  if c['noise_family']!='gaussian' and c['label'] in VULNERABLE_035:
   if R['wrong_wilson_upper_95']>.01 or (R['precision'] or 0)<.99 or R['wrong_acceptance']>T['wrong_acceptance']+.003:H['H6']=False
  if c['label'] in HIGH_NOISE_GAUSSIAN and R['coverage']<CW['coverage']+.08:H['H7']=False
  if c['label'] in HIGH_NOISE_GAUSSIAN and R['coverage']<M['coverage']-.02:H['H8']=False
  if losses[RADIAL_HUBER_STRATEGY]>losses[TRIAD]+.20:H['H9']=False
  fb=0
  if sum(int(float(x['causal_violation_count'])) for x in qs[RADIAL_HUBER_STRATEGY]) or sum(int(float(x['triad_veto_adapt_violations'])) for x in qs[RADIAL_HUBER_STRATEGY]):H['H10']=False
  for s in SEEDS:
   a,t=by[(s,RADIAL_HUBER_STRATEGY)],by[(s,TRIAD)]
 ...
 H['H11']=bool(abs(RADIAL_C-math.sqrt(-2.0*math.log(.05)))<1e-15 and BETA_STEP==.01 and BETA_MAX==1.20 and ACCEPT_THRESHOLD==.99 and WRONG_COST==100.0 and FALLBACK_COST==1.0)
 return {'evaluation_seeds':[39000,39999],'n_seeds_per_cell':1000,'cell_count':16,'strategies':STRATEGIES,'audit_seeds':sorted(AUDIT),'bootstrap_seed':39039,'bootstrap_resamples':10000,'hypotheses':H,'accept_threshold':ACCEPT_THRESHOLD,'wrong_action_cost':WRONG_COST,'fallback_cost':FALLBACK_COST,'radial_huber_cutoff':RADIAL_C,'cutoff_definition':'sqrt(chi2_2_0.95)=sqrt(-2 ln 0.05)','likelihood':'proper product of three normalized isotropic 2-D radial Huber densities on exact Experiment-028 covariance-whitened blocks','radial_huber_normalization_2d':'Z2(c)=2*pi*(1+exp(-c^2/2)/c^2)','whitening':'exact inherited Experiment-028 analytic 2x2 covariance whitening per directed-response block','context_rule':'Experiment-031 current-time causal context vote composed by Experiment-032','amplitude_prior':'HalfNormal inherited BETA_SCALE; trapezoid beta grid 0..1.20 step 0.01','vulnerable_experiment035_cells':sorted(VULNERABLE_035),'no_tuning':True,'code_commit':os.environ.get('GITHUB_SHA','unknown'),'cells':out}
