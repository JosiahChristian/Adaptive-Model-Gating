...
from experiment_056 import SIGNED_RANK_30_STRATEGY,W_CUTOFF,P345_NUMERATOR,P345_DENOMINATOR,P344_NUMERATOR,P344_DENOMINATOR
from run_experiment_056 import CELLS,STRATEGIES,SEEDS,AUDIT,calibration_values,exact_tail,run_experiment_056_strategy,summary

assert list(SEEDS)==list(range(56000,57000))
assert AUDIT==set(range(56000,56005))
...
c=CELLS[0];rows=run_experiment_056_strategy(56000,c,SIGNED_RANK_30_STRATEGY,vals);s=summary(rows,c)
assert int(float(s['rank55_contrast_count']))==30
assert int(float(s['rank55_spec_issue']))==196
for r in range(1,6):
...
print('Experiment 056 evaluator prospective smoke passed')
