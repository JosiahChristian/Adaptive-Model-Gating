...
def summary(rows,f,m):
 p=[r for r in rows if 401<=r['t']<=600];a=[r for r in rows if r['t']>=401];p20=[r for r in rows if 401<=r['t']<=420];r0=rows[0];target=BASELINE_A+m if f.startswith('drift') else BASELINE_A;avg=lambda k:sum(float(r.get(k,0)) for r in a)/len(a);pattern=f"{r0['inferred_group_a']}|{r0['inferred_group_b']}|{r0['inferred_group_c']}"
...
def report_from(summaries,tau,k,k3,la,lb,lc,lab,lac,lbc,ld):
 reps=[]
 for f,m in CELLS:
...
def calibrations():
 tau=calibrate_tau();k=calibrate_kappa();k3=calibrate_kappa3();la=calibrate_lambda_anchor();lb,lab=calibrate_dual_anchor_thresholds();lc,lac,lbc=calibrate_anchor_c_thresholds();ld=calibrate_lambda_dep();return tau,k,k3,la,lb,lc,lab,lac,lbc,ld
def main():
