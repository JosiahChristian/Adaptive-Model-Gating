...
from experiment_042 import LOCAL_GAUSSIAN_GROSS_STRATEGY,CLEAN_WEIGHT,GROSS_WEIGHT,VARIANCE_RATIO,MIXTURE_VARIANCE,CLEAN_SCALE,GROSS_SCALE,STRATEGIES,CELLS,run_experiment_042_strategy,evaluate_local_gaussian_gross_posterior
from run_experiment_021 import calibrations
SEEDS=range(42000,43000);AUDIT=set(range(42000,42005));Z=1.6448536269514722
...
HIGH_CONTAM={'contaminated_gaussian_g0.500_n1.50','contaminated_gaussian_g0.425_n1.50'}
...
def report_from(rows,posterior_rows):
 ...
 for c in CELLS:
  ...
  C,G,T,M,CW,RH,P,Q=rr[LOCAL_GAUSSIAN_GROSS_STRATEGY],rr[COMPOSED_STRATEGY],rr[ROBUST_STRATEGY],rr[MODEL_AVERAGED_STRATEGY],rr[HUBER_STRATEGY],rr[RADIAL_HUBER_STRATEGY],rr[LOCAL_MIXTURE_STRATEGY],rr[LOCAL_CAUCHY_STRATEGY]
  ...
  if c['noise_family']=='gaussian' and (C['coverage']<G['coverage']-.03 or losses[LOCAL_GAUSSIAN_GROSS_STRATEGY]>losses[COMPOSED_STRATEGY]+.05):H['H5']=False
  if c['noise_family']!='gaussian' and c['label'] in VULNERABLE_035:
   if C['wrong_wilson_upper_95']>.01 or (C['precision'] or 0)<.99 or C['wrong_acceptance']>T['wrong_acceptance']+.003:H['H6']=False
  if c['label'] in HIGH_CONTAM and (C['wrong_wilson_upper_95']>.01 or (C['precision'] or 0)<.99 or C['wrong_acceptance']>Q['wrong_acceptance'] or C['coverage']<Q['coverage']-.02):H['H7']=False
  if c['label'] in HIGH_GAUSS and (C['coverage']<Q['coverage']+.02 or C['wrong_wilson_upper_95']>.01 or (C['precision'] or 0)<.99):H['H8']=False
  if c['label'] in HIGH_GAUSS and C['coverage']<P['coverage']-.02:H['H9']=False
  if losses[LOCAL_GAUSSIAN_GROSS_STRATEGY]>losses[TRIAD]+.20:H['H10']=False
  fb=0
  if sum(int(float(x['causal_violation_count'])) for x in qs[LOCAL_GAUSSIAN_GROSS_STRATEGY]) or sum(int(float(x['triad_veto_adapt_violations'])) for x in qs[LOCAL_GAUSSIAN_GROSS_STRATEGY]):H['H11']=False
  for s in SEEDS:
   a,t=by[(s,LOCAL_GAUSSIAN_GROSS_STRATEGY)],by[(s,TRIAD)]
 ...
 H['H12']=bool(CLEAN_WEIGHT==.95 and GROSS_WEIGHT==.05 and VARIANCE_RATIO==25.0 and abs(MIXTURE_VARIANCE-2.2)<1e-12 and abs(CLEAN_SCALE-1/2.2)<1e-12 and abs(GROSS_SCALE-25/2.2)<1e-12 and BETA_STEP==.01 and BETA_MAX==1.20 and ACCEPT_THRESHOLD==.99 and WRONG_COST==100.0 and FALLBACK_COST==1.0)
 return {'evaluation_seeds':[42000,42999],'n_seeds_per_cell':1000,'cell_count':16,'strategies':STRATEGIES,'audit_seeds':sorted(AUDIT),'bootstrap_seed':42042,'bootstrap_resamples':10000,'hypotheses':H,'accept_threshold':ACCEPT_THRESHOLD,'wrong_action_cost':WRONG_COST,'fallback_cost':FALLBACK_COST,'local_mixture_weights':{'clean_gaussian':CLEAN_WEIGHT,'gross_gaussian':GROSS_WEIGHT},'gross_clean_variance_ratio':VARIANCE_RATIO,'covariance_normalization':MIXTURE_VARIANCE,'clean_covariance_scale':CLEAN_SCALE,'gross_covariance_scale':GROSS_SCALE,'likelihood':'proper independent per-block 0.95 N2(0,Sigma/2.2) + 0.05 N2(0,25 Sigma/2.2) gross-error mixture on three inherited 2-D directed-response covariance blocks','latent_marginalization':'component marginalized independently within each 2-D block; no posterior component selection','benchmark_origin':'0.95/0.05 weights and 25:1 variance ratio inherited from Experiment-035 contaminated-Gaussian family','benchmark_aligned_non_universal':True,'context_rule':'Experiment-031 current-time causal context vote composed by Experiment-032','amplitude_prior':'HalfNormal inherited BETA_SCALE; trapezoid beta grid 0..1.20 step 0.01','vulnerable_experiment035_cells':sorted(VULNERABLE_035),'contaminated_gaussian_mechanism_cells':sorted(HIGH_CONTAM),'high_noise_gaussian_recovery_cells':sorted(HIGH_GAUSS),'no_tuning':True,'code_commit':os.environ.get('GITHUB_SHA','unknown'),'cells':out}
