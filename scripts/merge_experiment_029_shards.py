...
from run_experiment_029 import CELLS,SEEDS,STRATEGIES,write_csv,report_from
SRC=ROOT/'results'/'experiment_029_shards';OUT=ROOT/'results'/'experiment_029';EXPECTED_SUMMARIES=15*3*1000;EXPECTED_AUDIT_ROWS=15*3*5*900
...
def main():
 ...
 for c in CELLS:
  ...
  if len(q)!=3000 or {int(r['seed']) for r in q}!=set(SEEDS) or {r['strategy'] for r in q}!=set(STRATEGIES):raise ValueError(c['label'])
 ...
 count=0
 with (OUT/'audit_seeds_29000_29004.csv').open('w',newline='',encoding='utf-8') as dst:
