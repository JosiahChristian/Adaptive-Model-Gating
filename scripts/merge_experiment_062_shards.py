...
from run_experiment_062 import CELLS,STRATEGIES,SEEDS,AUDIT,report_from,write_csv
    ...
    for p in paths:
        with p.open(encoding='utf-8') as f:
            for line in f:
                ...
                r=json.loads(line);row_count+=1;key=(str(r.get('experiment062_cell','')),int(r['seed']),str(r['strategy']))
...
def main():
    ...
    print(json.dumps({'summary_rows':len(rows),'audit_rows':audit_count,'audit_interval':interval,'H5_062_pass':report['H5_062_pass'],'H6_062_pass':report['H6_062_pass'],'integrity_pass':report['integrity_pass'],'interpretation_branch':report['interpretation_branch']},indent=2))
