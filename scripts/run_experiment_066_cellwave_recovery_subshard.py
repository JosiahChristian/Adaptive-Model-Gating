...
def main():
    ...
    chunk_start = full_start + DP_SHARD_N * RECOVERY_CHUNK
    start = chunk_start + RECOVERY_HALF_N * a.half_index
    stop = start + RECOVERY_HALF_N
    if not (chunk_start <= start < stop <= chunk_start + DP_SHARD_N):
        raise AssertionError('recovery_bounds')

    cell = CELLS[RECOVERY_CELL]
    ...
    meta = {
        ...
        'cell_index': RECOVERY_CELL,
        'chunk_index': RECOVERY_CHUNK,
