...
def _primary_one(seed, c):
    ...
    fallback_rows = e61.base.run_triad_persistence_on_stream(seed, f'experiment066_fallback_{c["label"]}', tau, k3, source_stream)
    deploy_rows = e61.base._run_composed_gate(seed, f'experiment066_deploy_{c["label"]}', tau, k3, la, lb, lc, lab, lac, lbc, source_stream, {}, candidate_groups)
    ...
    return {
        'panel': 'DP', 'seed': int(seed), 'cell': c['label'], 'topology_truth': truth,
        'discovery_candidate': candidate, 'candidate_correct': candidate_correct,
        'm1_accept': m1_accept, 'm1_correct': int(m1_accept and candidate_correct),
        'm1_wrong_accept': int(m1_accept and not candidate_correct), 'm1_e_value': float(m1['e_value']),
        'unanimous_match_count': int(m1['unanimous_match_count']), 'replicas': m1['replicas'],
        'a0_accept': a0_accept, 'a0_correct': int(a0_accept and candidate_correct),
        'a0_wrong_accept': int(a0_accept and not candidate_correct), 'a0_wplus': int(reconstructed_wplus),
        'a0_w_cutoff': int(e61.W_CUTOFF), 'a0_pairwise_confirmation': source_contrasts,
        'a0_ranks': tuple(int(x) for x in reconstructed_ranks), 'a0_confirmation_candidate': confirmation_candidate,
        'a0_confirmation_profile': tuple(float(x) for x in y_conf), 'a0_confirmation_scores': source_topology_scores,
        'a0_topology_tie_flag': source_tie, 'a0_topology_agreement': int(confirmation_candidate == candidate),
        'fallback_operational_loss_401_600': fallback_loss, 'deploy_candidate_operational_loss_401_600': deploy_loss,
        ...
        'source_discovery_stream_shared_with_a0': True, 'replica_discovery_used': False, 'no_tuning': True,
    }
...
def main():
    ...
    if a.panel:
        if a.cell_index is not None or a.chunk_index is not None:
        ...
        meta = {
            ...
            'seed_range': [start, stop - 1], 'n': len(rows), 'replica_count': 5,
            'operative_spec_issue': 269, 'provenance_closure_issue': 270, 'preflight_issue': 271,
        ...
        if a.chunk_index is None or not 0 <= a.chunk_index < DP_SHARD_COUNT:
        ...
        start = full_start + a.chunk_index * DP_SHARD_N
        stop = start + DP_SHARD_N
        c = CELLS[a.cell_index]
        rows = [_primary_one(seed, c) for seed in range(start, stop)]
        if [r['seed'] for r in rows] != list(range(start, stop)) or len(rows) != DP_SHARD_N:
        ...
        meta = {
            'experiment': 66, 'role': 'development_primary_shard', 'cell_index': a.cell_index,
            'chunk_index': a.chunk_index, 'cell': c['label'], 'seed_range': [start, stop - 1],
            'n': len(rows), 'paired_m1_a0': True, 'replica_count': 5,
            'extra_m1_confirmation_scalars_per_source_draw': EXTRA_M1_CONFIRMATION_SCALARS,
            'operative_spec_issue': 269, 'provenance_closure_issue': 270, 'preflight_issue': 271,
            'execution_closure_issue': EXECUTION_CLOSURE_ISSUE,
            'validation_seeds_touched': False, 'no_tuning': True,
        }
