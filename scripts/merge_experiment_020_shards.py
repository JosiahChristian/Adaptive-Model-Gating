...
from run_experiment_020 import CELLS,SEEDS,STRATEGIES,write_csv,report_from
SRC=ROOT/'results'/'experiment_020_shards';OUT=ROOT/'results'/'experiment_020'
EXPECTED_SUMMARIES=28*16*200
EXPECTED_AUDIT_ROWS=28*16*5*900

def readcsv(p):
 with p.open(encoding='utf-8',newline='') as f:return list(csv.DictReader(f))
def num(r):
 for k,v in list(r.items()):
  if k in {'family','strategy','early_selected_edge'}:continue
  try:r[k]=float(v)
  except:pass
 return r
def stream_audit(paths,out):
 ...
 with out.open('w',encoding='utf-8',newline='') as dst:
  ...
  for p in paths:
   ...
   if shard_count!=16*5*900:raise ValueError(f'Expected 72000 audit rows in {p}, got {shard_count}')
 return count
def main():
 ...
 for f,m in CELLS:
  ...
  if len(c)!=16*200 or {int(r['seed']) for r in c}!=set(SEEDS) or {r['strategy'] for r in c}!=set(STRATEGIES):raise ValueError(f'Coverage mismatch {f} {m}')
 first=metas[0]
 for x in metas[1:]:
  for k in ('mu_4_early','nu_4_early','early_calibration_seeds','evaluation_seeds'):
 ...
 audit_count=stream_audit(audit_paths,OUT/'audit_trace_seeds_20000_20004.csv')
