...
from experiment_055 import SIGNED_RANK_30_STRATEGY,W_CUTOFF,P345_NUMERATOR,P345_DENOMINATOR,P344_NUMERATOR,P344_DENOMINATOR
from run_experiment_055 import CELLS,STRATEGIES,SEEDS,AUDIT,calibration_values,exact_tail,run_experiment_055_strategy,summary

assert list(SEEDS)==list(range(55000,56000))
assert AUDIT==set(range(55000,55005))
...
c=CELLS[0];rows=run_experiment_055_strategy(55000,c,SIGNED_RANK_30_STRATEGY,vals);s=summary(rows,c)
assert int(float(s['rank55_contrast_count']))==30
assert int(float(s['rank55_spec_issue']))==190
for r in range(1,6):
...
print('Experiment 055 evaluator prospective smoke passed')
