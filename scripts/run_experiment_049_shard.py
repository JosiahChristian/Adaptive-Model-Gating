...
from experiment_049 import CELLS,STRATEGIES,run_experiment_049_strategy
from run_experiment_049 import SEEDS,AUDIT,summary,write_csv,calibration_values
...
def main():
 ...
 with (out/'audit.jsonl').open('w',encoding='utf-8') as af:
  for seed in SEEDS:
   for st in STRATEGIES:
    rows=run_experiment_049_strategy(seed,c,st,vals);summ.append(summary(rows,c))
    if seed in AUDIT:
 ...
 if len(summ)!=12000:raise AssertionError(len(summ))
