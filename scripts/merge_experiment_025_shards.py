...
from run_experiment_025 import CELLS,SEEDS,STRATEGIES,write_csv,report_from
SRC=ROOT/'results'/'experiment_025_shards';OUT=ROOT/'results'/'experiment_025';EXPECTED_SUMMARIES=46*5*200;EXPECTED_AUDIT_ROWS=46*5*5*900

def readcsv(p):
 with p.open(encoding='utf-8',newline='') as f:return list(csv.DictReader(f))
def num(r):
 for k,v in list(r.items()):
  if k in {'label','strategy','adapt_signature','gate_signature'}:continue
  try:r[k]=float(v)
  except:pass
 return r
def main():
 ...
 for c in CELLS:
  ...
  if len(q)!=1000 or {int(r['seed']) for r in q}!=set(SEEDS) or {r['strategy'] for r in q}!=set(STRATEGIES):raise ValueError(c['label'])
 ...
 count=0
 with (OUT/'audit_trace_seeds_25000_25004.csv').open('w',newline='',encoding='utf-8') as dst:
