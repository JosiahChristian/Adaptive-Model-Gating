...
from run_experiment_063 import CELLS,STRATEGIES,SEEDS,AUDIT,report_from,write_csv
    ...
    for p in paths:
        with p.open(encoding='utf-8') as f:
            for line in f:
                ...
                r=json.loads(line);row_count+=1;key=(str(r.get('experiment063_cell','')),int(r['seed']),str(r['strategy']))
...
def main():
    ...
    report=report_from(rows);report['summary_row_count']=len(rows);report['audit_row_count']=audit_count;report['audit_time_interval']=interval
    (out/'report.json').write_text(json.dumps(report,indent=2,sort_keys=True),encoding='utf-8')
    print(json.dumps({'summary_rows':len(rows),'audit_rows':audit_count,'audit_interval':interval,'H5_063_pass':report['H5_063_pass'],'H6_063_pass':report['H6_063_pass'],'R_063_pass':report['R_063_pass'],'integrity_pass':report['integrity_pass'],'interpretation_branch':report['interpretation_branch']},indent=2))
