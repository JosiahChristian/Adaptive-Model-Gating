...
from run_experiment_032 import CELLS,report_from,calibration_values,write_csv

def main():
 base=ROOT/'results'/'experiment_032_shards';rows=[]
 for c in CELLS:
  p=base/c['label']/'seed_summary.csv'
  if not p.exists():raise FileNotFoundError(p)
  with p.open(newline='',encoding='utf-8') as f:rows.extend(csv.DictReader(f))
 if len(rows)!=51000:raise ValueError(f'expected 51000 summaries, got {len(rows)}')
 out=ROOT/'results'/'experiment_032';out.mkdir(parents=True,exist_ok=True);write_csv(out/'seed_summary.csv',rows)
 audit_count=0
 ...
 if audit_count!=229500:raise ValueError(f'expected 229500 audit rows, got {audit_count}')
