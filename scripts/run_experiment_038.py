...
from experiment_038 import HUBER_STRATEGY,HUBER_C,STRATEGIES,CELLS,run_experiment_038_strategy,evaluate_huber_posterior
from run_experiment_021 import calibrations
SEEDS=range(38000,39000);AUDIT=set(range(38000,38005));Z=1.6448536269514722
...
def report_from(rows,posterior_rows):
 ...
 for c in CELLS:
  ...
  HBR,G,T,M=rr[HUBER_STRATEGY],rr[COMPOSED_STRATEGY],rr[ROBUST_STRATEGY],rr[MODEL_AVERAGED_STRATEGY]
  if HBR['wrong_wilson_upper_95']>.01:H['H2']=False
  if (HBR['precision'] or 0)<.99:H['H3']=False
  if float(c['noise_scale'])==1.0:
   req=.90 if float(c['gain'])==.50 else .85
   if HBR['coverage']<req:H['H4']=False
  losses={st:avg(qs[st],'operational_loss_401_600') for st in STRATEGIES}
  if c['noise_family']=='gaussian' and (HBR['coverage']<G['coverage']-.03 or losses[HUBER_STRATEGY]>losses[COMPOSED_STRATEGY]+.05):H['H5']=False
  if c['noise_family']!='gaussian' and c['label'] in VULNERABLE_035:
   if HBR['wrong_wilson_upper_95']>.01 or (HBR['precision'] or 0)<.99 or HBR['wrong_acceptance']>T['wrong_acceptance']+.003:H['H6']=False
  if c['label'] in ('gaussian_g0.500_n1.50','gaussian_g0.425_n1.50') and HBR['coverage']<M['coverage']+.02:H['H7']=False
  if losses[HUBER_STRATEGY]>losses[TRIAD]+.20:H['H8']=False
  fb=0
  if sum(int(float(x['causal_violation_count'])) for x in qs[HUBER_STRATEGY]) or sum(int(float(x['triad_veto_adapt_violations'])) for x in qs[HUBER_STRATEGY]):H['H9']=False
  for s in SEEDS:
   a,t=by[(s,HUBER_STRATEGY)],by[(s,TRIAD)]
 ...
 H['H10']=bool(HUBER_C==1.345 and BETA_STEP==.01 and BETA_MAX==1.20 and ACCEPT_THRESHOLD==.99 and WRONG_COST==100.0 and FALLBACK_COST==1.0)
 return {'evaluation_seeds':[38000,38999],'n_seeds_per_cell':1000,'cell_count':16,'strategies':STRATEGIES,'audit_seeds':sorted(AUDIT),'bootstrap_seed':38038,'bootstrap_resamples':10000,'hypotheses':H,'accept_threshold':ACCEPT_THRESHOLD,'wrong_action_cost':WRONG_COST,'fallback_cost':FALLBACK_COST,'huber_cutoff':HUBER_C,'likelihood':'proper product Huber density on exact block-whitened Experiment-028 directed residuals','amplitude_prior':'HalfNormal inherited BETA_SCALE; trapezoid beta grid 0..1.20 step 0.01','vulnerable_experiment035_cells':sorted(VULNERABLE_035),'no_tuning':True,'code_commit':os.environ.get('GITHUB_SHA','unknown'),'cells':out}
