...
from experiment_036 import ROBUST_STRATEGY,STRATEGIES,CELLS,NU,BETA_MAX,BETA_STEP,run_experiment_036_strategy,evaluate_robust_posterior
from run_experiment_021 import calibrations
SEEDS=range(36000,37000);AUDIT=set(range(36000,36005));Z=1.6448536269514722
VULNERABLE_035={
'contaminated_gaussian_g0.425_n1.00','contaminated_gaussian_g0.425_n1.50','contaminated_gaussian_g0.500_n1.00','contaminated_gaussian_g0.500_n1.50',
'student_t3_g0.425_n1.00','student_t3_g0.425_n1.50','student_t3_g0.500_n1.00','student_t3_g0.500_n1.50','laplace_g0.425_n1.50'}
...
def report_from(rows,posterior_rows):
 ...
 for c in CELLS:
  ...
  R=rr[ROBUST_STRATEGY];G=rr[COMPOSED_STRATEGY]
  ...
  if c['noise_family']=='gaussian' and (R['coverage']<G['coverage']-.03 or losses[ROBUST_STRATEGY]>losses[COMPOSED_STRATEGY]+.05):H['H5']=False
  legacy=c['label'].replace('gaussian_','')
  if c['noise_family']!='gaussian' and c['label'] in VULNERABLE_035:
   if not (R['wrong_acceptance']<G['wrong_acceptance'] and R['wrong_wilson_upper_95']<=.01 and (R['precision'] or 0)>=.99):H['H6']=False
  if losses[ROBUST_STRATEGY]>losses[TRIAD]+.20:H['H7']=False
  if sum(int(float(x['causal_violation_count'])) for x in qs[ROBUST_STRATEGY]) or sum(int(float(x['triad_veto_adapt_violations'])) for x in qs[ROBUST_STRATEGY]):H['H8']=False
  fb=0
  for s in SEEDS:
   a,t=by[(s,ROBUST_STRATEGY)],by[(s,TRIAD)]
 ...
 H['H10']=bool(NU==3.0 and BETA_STEP==.01 and BETA_MAX==1.20 and ACCEPT_THRESHOLD==.99 and WRONG_COST==100.0 and FALLBACK_COST==1.0)
 return {'evaluation_seeds':[36000,36999],'n_seeds_per_cell':1000,'cell_count':16,'strategies':STRATEGIES,'audit_seeds':sorted(AUDIT),'bootstrap_seed':36036,'bootstrap_resamples':10000,'hypotheses':H,'accept_threshold':ACCEPT_THRESHOLD,'wrong_action_cost':WRONG_COST,'fallback_cost':FALLBACK_COST,'robust_likelihood':'multivariate Student-t df=3; inherited covariance matched via scatter=Sigma/3','amplitude_prior':'HalfNormal inherited BETA_SCALE; trapezoid beta grid 0..1.20 step 0.01','vulnerable_experiment035_cells':sorted(VULNERABLE_035),'no_tuning':True,'code_commit':os.environ.get('GITHUB_SHA','unknown'),'cells':out}
