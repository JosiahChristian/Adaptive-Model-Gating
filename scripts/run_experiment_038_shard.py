...
from experiment_038 import CELLS,STRATEGIES,run_experiment_038_strategy,evaluate_huber_posterior
from run_experiment_038 import SEEDS,AUDIT,summary,write_csv,calibration_values
...
def main():
 ...
 with (out/'audit.jsonl').open('w',encoding='utf-8') as af:
  for seed in SEEDS:
   post.extend(evaluate_huber_posterior(seed,c))
   for st in STRATEGIES:
    rows=run_experiment_038_strategy(seed,c,st,vals);summ.append(summary(rows,c))
    if seed in AUDIT:
 ...
 if len(summ)!=5000 or len(post)!=5000:raise AssertionError((len(summ),len(post)))
