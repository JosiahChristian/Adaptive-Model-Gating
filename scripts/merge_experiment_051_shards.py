...
def validate_audit(lines):
 ...
 groups={}
 for r in records:
  label=str(r.get('experiment051_cell',''))
  seed=int(r['seed'])
  st=str(r['strategy'])
  key=(label,seed,st)
 ...
 if set(groups)!=expected:
  missing=sorted(expected-set(groups));extra=sorted(set(groups)-expected)
  raise AssertionError(('audit_groups',len(groups),len(expected),missing[:5],extra[:5]))
 for key,ts in groups.items():
  if len(ts)!=len(set(ts)):raise AssertionError(('duplicate_audit_t',key))
  ordered=sorted(ts)
  if ordered!=list(range(1,ordered[-1]+1)):
   raise AssertionError(('noncontiguous_audit_t',key,ordered[:3],ordered[-3:]))
 return records
...
def main():
 ...
 expected_summary=len(CELLS)*len(SEEDS)*len(STRATEGIES)
