...
import experiment_062 as exp62
import run_experiment_055 as base
from experiment_051 import CELLS

SEEDS=range(70000,71000)
AUDIT=set(range(70000,70005))
STRATEGIES=exp62.STRATEGIES
PRIMARY_STRATEGY=exp62.CONFIRMATION_AGREEMENT_STRATEGY
COMPARATOR_STRATEGY=exp62.SIGNED_RANK_30_STRATEGY
W_CUTOFF=exp62.W_CUTOFF
P345_NUMERATOR=exp62.P345_NUMERATOR
P345_DENOMINATOR=exp62.P345_DENOMINATOR
P344_NUMERATOR=exp62.P344_NUMERATOR
P344_DENOMINATOR=exp62.P344_DENOMINATOR
...
def run_experiment_062_strategy(seed,c,strategy,vals):
    return exp62.run_experiment_062_strategy(seed,c,strategy,vals)
...
def summary(rows,c):
    ...
    if r0['strategy']==PRIMARY_STRATEGY:
        for k,v in r0.items():
            if str(k).startswith('rank55_') or str(k).startswith('rank61_') or str(k).startswith('rank62_'):
                out[k]=v
    return out
...
def integrity(x):
    if int(float(x.get('rank62_spec_issue',0) or 0))!=OPERATIVE_SPEC_ISSUE:return False
    if int(float(x.get('rank62_contrast_count',0) or 0))!=30:return False
    if int(float(x.get('rank62_w_cutoff',0) or 0))!=W_CUTOFF:return False
    if int(float(x.get('rank62_no_extra_observations',0) or 0))!=1:return False
    if int(float(x.get('rank62_candidate_reselected',1)))!=0:return False
    ...
    w,ranks=exp62.signed_rank_statistic_30(vals)
    if w!=int(float(x.get('rank62_wplus',-1))):return False
    ...
    if int(float(x.get('rank62_signed_rank_pass',-1)))!=signed_pass:return False
    disc=str(x.get('rank62_candidate',''))
    conf=str(x.get('rank62_confirmation_candidate',''))
    ...
    if int(float(x.get('rank62_confirmation_agreement',-1)))!=agreement:return False
    accepted=int(signed_pass and agreement)
    if int(float(x['coverage']))!=accepted:return False
    if abs(float(x.get('rank61_e_final',0))-(exp62.ACCEPT_E if accepted else 0.0))>1e-12:return False
    return True
...
def report_from(rows):
    ...
    for c in CELLS:
        ...
        agreement=sum(int(float(x.get('rank62_confirmation_agreement',0) or 0)) for x in primary)/len(primary)
        ...
        cells[label]={
            ...
            'H5_062_cell_pass':h5_cell,
            'H6_062_cell_pass':h6_cell,
    ...
    return {
        ...
        'accept_e':exp62.ACCEPT_E,
        'repair_rule':'exact replication of Experiment 061: deploy iff unchanged W+>=345 passes for discovery-selected candidate and confirmation-only topology argmax agrees; agreement is veto-only',
        'utility_role':'descriptive only in Experiment 062',
        'exact_replication_of_experiment061':True,
