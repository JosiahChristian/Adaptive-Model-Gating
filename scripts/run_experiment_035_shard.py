...
from experiment_035 import CELLS,STRATEGIES,run_experiment_035_strategy,evaluate_experiment_035_posterior
from run_experiment_035 import SEEDS,AUDIT,summary,write_csv,calibration_values
...
def main():
 ...
 with (out/'audit.jsonl').open('w',encoding='utf-8') as af:
  for seed in SEEDS:
   post.extend(evaluate_experiment_035_posterior(seed,c))
   for st in STRATEGIES:
    rows=run_experiment_035_strategy(seed,c,st,vals);summ.append(summary(rows,c))
    if seed in AUDIT:
