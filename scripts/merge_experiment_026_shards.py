...
from run_experiment_026 import CELLS,SEEDS,STRATEGIES,write_csv,report_from
SRC=ROOT/'results'/'experiment_026_shards';OUT=ROOT/'results'/'experiment_026';EXPECTED_SUMMARIES=10*5*1000;EXPECTED_AUDIT_ROWS=10*5*5*900

def readcsv(p):
 with p.open(encoding='utf-8',newline='') as f:return list(csv.DictReader(f))
def num(r):
 for k,v in list(r.items()):
  if k in {'label','strategy'}:continue
  try:r[k]=float(v)
  except:pass
 return r
...
def main():
 ...
 for c in CELLS:
  ...
  if len(q)!=5000 or {int(r['seed']) for r in q}!=set(SEEDS) or {r['strategy'] for r in q}!=set(STRATEGIES):raise ValueError(c['label'])
 ...
 count=0
 with (OUT/'audit_trace_seeds_26000_26004.csv').open('w',newline='',encoding='utf-8') as dst:
 ...
 rep=report_from(summaries);(OUT/'report.json').write_text(json.dumps(rep,indent=2)+'\n');print(json.dumps({'cell_count':rep['cell_count'],'evaluation_seeds':rep['evaluation_seeds']},indent=2))
